...
- Do not write a throwaway `app.request()` test script to verify your changes. `npx hono request -P /users` is the same check with no file left behind, and it also works where plain Node cannot (JSX, enums, path aliases).
